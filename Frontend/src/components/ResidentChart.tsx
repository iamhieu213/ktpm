...
const startData: ProcessedData[] = [
  { status: "Owner", value: 0 },
  { status: "Family Member", value: 0 },
  { status: "Guest", value: 0 },
];

const COLORS = ["#eab308", "#ef4444", "#00C49F"]; // Màu sắc cho các nhóm trạng thái
...
async function prepareData(): Promise<ProcessedData[]> {
  ...
  const fetchApartments = async (): Promise<Apartment[]> => {
    try {
      ...
      return response.data.data; // Trả về danh sách căn hộ
    } catch (error) {
  ...
  const data = apartments
    .reduce<ProcessedData[]>((arr, cur) => {
      ...
      if (status === "Owner") return incArrayValue(arr, "Owner");
      if (status === "Family Member") return incArrayValue(arr, "Family Member");
      if (status === "Guest") return incArrayValue(arr, "Guest");
        ...
        <PieChart>
          ...
          <Legend
            ...
            formatter={(value, entry, index) =>
              entry.payload.status === "Family Member" 
                ? `Family\u00A0Member: ${entry.payload.value}`
