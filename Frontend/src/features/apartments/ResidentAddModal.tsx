...
export default function ResidentAddModal({ onResidentsSelect }: ResidentAddModalProps) {
  ...
  const [addedIdResident, setAddedIdResident] = useState<string[]>([]); // Chỉ lưu danh sách id cho gợi ý

  // Hàm gọi API để lấy danh sách residents
  const apiAllResidents = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/residents?size=100&page=1`);
      ...
      const residentsList = residentsData.map((resident: any) => ({
        id: resident.id,
        name: resident.name,
        dob: resident.dob, // Hoặc resident.dob, tùy API
      }));
      ...
      setAddedIdResident(residentsList.map((resident) => resident.id))
    } catch (err) {
  ...
  return (
    <div className="wrp">
      <div className="searchDiv">
        <p className="text-cpn">Search Residen  t:</p>
